...
namespace OutlookStyleControls
{
  ...
  public partial class OutlookGrid : DataGridView
  {
    ...
    private void SetupColumns()
    {
      ...
      // Attempt to save the columns first. This may throw an error in some cases, we just swallow them...
      Dictionary<string, cDataRowMapper> old_cols = new Dictionary<string, cDataRowMapper>();
      try
      {
        foreach (DataGridViewTextBoxColumn row in Columns)
          old_cols.Add(row.DataPropertyName, new cDataRowMapper(row.HeaderText, row.Visible));
      }
      catch { }
      ...
      {
        cDataRowMapper mapper;
        old_cols.TryGetValue(c, out mapper);
        ...
        if (mapper != null)
        {
          Columns[index].Visible = mapper.Visible;
          Columns[index].HeaderText = mapper.HeaderText;
        }
      }
    ...
    private void FillGrid(IOutlookGridGroup groupingStyle)
    {
      ...
      else
      {
        ...
        foreach (DataSourceRow r in list)
        {
          ...
          if (groupCur.BackColor != null)
            row.DefaultCellStyle.BackColor = groupCur.BackColor;
          Rows.Add(row);
          groupCur.ItemCount = counter;
        }
      }
  ...
  class cDataRowMapper
  {
    public cDataRowMapper(string aHeaderText, bool aVisible)
    {
